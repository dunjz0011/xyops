...
class xyOps extends Component {
	...
	startup(callback) {
		...
		this.logDebug(3, "PTOps engine starting up", process.argv );
	...
	finishStartup(callback) {
		...
		this.request = new PixlRequest( "PTOps v" + this.server.__version );
	...
	shutdown(callback) {
		...
		var self = this;
		this.shut = true;
		this.logDebug(2, "Shutting down PTOps");
		this.multiShutdown();
		this.maintShutdown();
