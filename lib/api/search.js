...
class Search {
	...
	api_marketplace(args, callback) {
		// make search or fetch request to the PTOps marketplace system
		...
		this.loadSession(args, function (err, session, user) {
			...
			if (!marketplace.enabled) {
				return self.doError('marketplace', "The marketplace system is not enabled.  Please enable it in your PTOps configuration and try again.", callback);
			}
	...
	api_bulk_search_export(args, callback) {
		...
		this.loadSession(args, function (err, session, user) {
			...
			var filename = 'ptops-' + params.index + '-export-' + Tools.formatDate(Tools.timeNow(), '[yyyy]-[mm]-[dd]') + '-' + Tools.generateShortID() + formatter.file_ext;
			var job = null;
			var stream = null;
