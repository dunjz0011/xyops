...
class Jobs {
	...
	api_download_job_log(args, callback) {
		...
		if (job) {
			...
			var headers = {
				'Content-Type': "text/plain; charset=utf-8",
				'Content-disposition': "attachment; filename=PTOps-Partial-Job-Log-" + params.id + '.txt'
			};
		...
		this.unbase.get( "jobs", params.id, function(err, job) {
			...
			var headers = {
				'Content-Type': "text/plain; charset=utf-8",
				'Content-disposition': "attachment; filename=PTOps-Job-Log-" + params.id + '.txt'
			};
